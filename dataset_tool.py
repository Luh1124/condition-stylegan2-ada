...
def open_image_folder(source_dir, *, max_images: Optional[int]):
    ...
    input_images = [str(f) for f in sorted(Path(source_dir).rglob('*')) if is_image_ext(f) and os.path.isfile(f) and f.split(os.sep)[-3]=="unwarp_texture"]
...
@click.command()
@click.pass_context
@click.option('--source', help='Directory or archive name for input dataset', default='/media/lh/6T空盘/lh_6T/faceset/test_set/unwrap_texture', metavar='PATH')
@click.option('--dest', help='Output directory or archive name for output dataset', default='/media/lh/6T空盘/lh_6T/faceset/test_set/test_image', metavar='PATH')
